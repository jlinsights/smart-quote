...
const labels = {
  en: {
    ...
    welcome: (name?: string) =>
      name
        ? `Hi ${name}! How can I help you with your shipping quote today?`
        : 'Hi there! How can I help you with your shipping quote today?',
  ...
  ko: {
    ...
    welcome: (name?: string) =>
      name
        ? `안녕하세요 ${name}님! 배송 견적에 대해 도움이 필요하신가요?`
        : '안녕하세요! 배송 견적에 대해 도움이 필요하신가요?',
  ...
  ja: {
    ...
    welcome: (name?: string) =>
      name
        ? `こんにちは ${name}さん！配送見積もりについてお手伝いしますか？`
        : 'こんにちは！配送見積もりについてお手伝いしますか？',
...
export const AiChatWidget: React.FC = () => {
  ...
  const handleOpen = useCallback(() => {
    ...
    if (!hasGreeted) {
      setMessages([
        { role: 'assistant', content: l.welcome(user?.name) },
      ]);
      setHasGreeted(true);
    }
    ...
    <>
      ...
      <div
        className={`fixed bottom-24 right-5 z-50 flex flex-col
          w-[calc(100vw-2.5rem)] max-w-[380px] h-[520px]
          sm:w-[380px]
          max-sm:inset-0 max-sm:w-full max-sm:h-full max-sm:max-w-none max-sm:bottom-0 max-sm:right-0 max-sm:rounded-none
          bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700
          transition-all duration-200 ease-out origin-bottom-right
          ${isOpen ? 'scale-100 opacity-100 pointer-events-auto' : 'scale-95 opacity-0 pointer-events-none'}`}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 bg-gradient-to-r from-jways-600 to-jways-700 text-white rounded-t-2xl max-sm:rounded-t-none shrink-0">
        ...
        <div className="shrink-0 px-4 py-3 border-t border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2.5 rounded-full bg-jways-600 hover:bg-jways-700 text-white disabled:opacity-40 disabled:cursor-not-allowed transition-colors shrink-0"
      ...
      <button
        ...
        className={`fixed bottom-5 right-5 z-50 w-14 h-14 rounded-full bg-jways-600 hover:bg-jways-700 text-white shadow-lg flex items-center justify-center transition-all duration-200 ${
