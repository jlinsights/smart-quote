import { 
  ...
  const determineUpsZone = (country: string, zip: string): { zone: string; label: string } => {
     ...
     if (country === 'CN') {
         ...
         // Range Logic for Zone 5 (South China):
         // 350000-369999 (Fujian)
         // 510000-539999 (Guangdong/Guangxi)
         ...
         if (!isNaN(zipNum)) {
            for (const range of CN_SOUTH_ZIP_RANGES) {
                if (zipNum >= range.start && zipNum <= range.end) {
                    return { 
                        zone: 'Zone 5', 
                        label: 'Zone 5 (South China - Fujian/Guangdong/Hainan/Yunnan)' 
                    };
                }
            }
         }
         // Default for China if no range matched (Beijing, Shanghai, etc. are typically Zone 2)
